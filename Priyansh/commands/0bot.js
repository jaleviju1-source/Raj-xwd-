...
module.exports.run = async function ({ api, event, args }) {
  ...
  if (!cleanedMsg) return api.sendMessage("chal hawa aane de bhalu 🦍🐻", threadID, messageID);
  ...
  if (isVideoReq || isAudioReq || isUrl) {
    try {
      ...
      let query = cleanedMsg.replace(/video|vdo|mp4|song|music|audio|photo images|mp3|play|gaana|gane|ghana/gi, "").trim();
      ...
      // API Request
      const response = await axios.get(PRIYANSHU_API_URL, {
        params: {
          url: videoURL,
          format: format,
          apikey: PRIYANSHU_API_KEY
        },
        timeout: 20000
      });

      const resData = response.data;
      const downloadUrl = resData.downloadLink || resData.url || resData.result || resData.data?.downloadLink || resData.data?.url;

      if (!downloadUrl) {
        console.error("API Response Error:", resData);
      ...
      const infoMsg = `🖤 𝗧𝗶𝘁𝗹𝗲: ${video.title}\n\n${OWNER_TAG}\n🥀𝒀𝑬 𝑳𝑶 𝑩𝑨𝑩𝒀 𝑨𝑷𝑲𝗜👉 ${format.toUpperCase()}`;
      ...
      writer.on("error", (err) => {
        ...
        api.setMessageReaction("❌", messageID, () => {}, true);
        api.sendMessage("File save karte waqt masla ho gaya 🥺", threadID, messageID);
      });
      ...
      api.setMessageReaction("❌", messageID, () => {}, true);
      return api.sendMessage("Server thoda thak gaya hai ya API mein issue hai, baad mein try karo 🥺", threadID, messageID);
    }
  }
  ...
  const prompt = `System Command: Tum bot ho, ek real aur cute larki. Sirf Roman Urdu mein bilkul natural baat karo, jaise hum aam zindgi mein karte hain.

Owner UID: 61592190219130

Owner Recognition:
- Agar UID "61592190219130" hai, to woh Shaan hai.
